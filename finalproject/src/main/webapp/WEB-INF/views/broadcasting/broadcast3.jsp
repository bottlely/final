...
<link rel="stylesheet" type="text/css" href="stylecss/style_chat.css">
<style type="text/css">
.user-photo {
 width: 60px;
 height: 60px;
...
<div class="chatbox">
	<div class="chatlogs">
		<div id="messageWindow2" class="chat friend">
			<!-- onkeydown을 통해서 엔터키로도 입력되도록 설정. -->
			<div class="user-photo"><img src="myHomeFolder/profile_img/default-profile.jpg"></div>
			<p class="chat-message" id="from_text"></p>
			<!-- <input id="inputMessage" type="text" onkeydown="if(event.keyCode==13){send();}" /> -->
		</div>
		<div id="messageWindow2" class="chat self">
			<!-- onkeydown을 통해서 엔터키로도 입력되도록 설정. -->
			<div class="user-photo"></div>
			<p class="chat-message" id="to_text"></p>
			<!-- <input id="inputMessage" type="text" onkeydown="if(event.keyCode==13){send();}" /> -->
		</div>
	</div>
	<div class="chat-form">
		<textarea id="inputMessage" onkeydown="if(event.keyCode==13){send();}"></textarea>
		<input type="submit" value="send" onclick="send();" id="chat-form-button">
	</div>
</div>
...
<script type="text/javascript">
	...
	function onMessage(event) {
		...
		var message = event.data.split("|\|");
			...
			//div는 받은 메시지 출력할 공간.
			var div1=document.getElementById('from_text').value;
			var div=document.createElement('div');
